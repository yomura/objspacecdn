...
class ObjSpaceCDN_DHO_Services extends ObjSpaceCDN_Plugin_Base {
	...
	const SETTINGS_KEY = 'obspacecdn_settings';

	function __construct( $plugin_file_path ) {
		parent::__construct( $plugin_file_path );

		do_action( 'obspacecdn_init', $this );

		if ( is_admin() ) {
			do_action( 'obspacecdn_admin_init', $this );
		}

		add_action( 'admin_menu', array( $this, 'admin_menu' ) );
		$this->plugin_permission = 'manage_options';
		$this->plugin_vars = array( 'access_key_id', 'secret_access_key' );
		$this->plugin_title = __( 'ObjSpace CDN', 'objspacecdn' );
		$this->plugin_menu_title = __( 'ObjSpaceCDN', 'objspacecdn' );
	}
	...
	function admin_menu() {
    	...
    	if ( isset( $submenu[$this->plugin_slug][0][0] ) ) {
    		$submenu[$this->plugin_slug][0][0] = __( 'Settings', 'objspacecdn' );
		}

		$title = __( 'Settings', 'objspacecdn' );
	...
	function plugin_load() {
		...
		wp_enqueue_script( 'objspacecdn-script', $src, array( 'jquery' ), $this->get_installed_version(), true );

		$this->handle_post_request();

		do_action( 'ObjSpaceCDN_Plugin_load', $this );
	}
	...
	function handle_post_request() {
		...
		if ( empty( $_POST['_wpnonce'] ) || !wp_verify_nonce( $_POST['_wpnonce'], 'objspacecdn-save-settings' ) ) {
	...
	function get_client() {
		if ( !$this->get_access_key_id() || !$this->get_secret_access_key() ) {
			return new WP_Error( 'access_keys_missing', sprintf( __( '<div class="dashicons dashicons-no"></div> Please <a href="%s">set your access keys</a> first.', 'objspacecdn' ), 'admin.php?page=' . $this->plugin_slug ) );
		}
